 ...
 *
 * File: esp32_connect.c ESP32 Wi-Fi AP connect and smart config.
 *
...
static const char *NVS_WIFI_CONFIG = "wifi config";
static const char *NVS_WIFI_CONFIG_DATA = "data";
...
esp_err_t example_connect(void)
{
    ...
    for (int i=0; i<NR_OF_IP_ADDRESSES_TO_WAIT_FOR; ++i) {
        xSemaphoreTake(s_semph_get_ip_addrs, 5000 / portTICK_RATE_MS);
    }
...
static esp_netif_t* wifi_start(void)
{
    ...
    err = nvs_open(NVS_WIFI_CONFIG, NVS_READWRITE, &handle);
    if (err == ESP_OK) {
        err = nvs_get_blob(handle, NVS_WIFI_CONFIG_DATA, &wifi_config, &len);
        if (err == ESP_ERR_NVS_NOT_FOUND) {
            ESP_LOGI(TAG, "nvs wifi config not found!");
        ...
        ESP_LOGI(TAG, "Failed to open nvs wifi config!");
    }
                         ...
                         int32_t event_id, void* event_data)
{
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
        ...
        nvs_handle handle;
        ESP_ERROR_CHECK( nvs_open(NVS_WIFI_CONFIG, NVS_READWRITE, &handle) );
        ESP_ERROR_CHECK( nvs_set_blob(handle, NVS_WIFI_CONFIG_DATA, &wifi_config, sizeof(wifi_config)) );
...
esp_err_t example_smart_config(void)
{
    ESP_ERROR_CHECK( esp_event_handler_unregister(WIFI_EVENT, WIFI_EVENT_STA_DISCONNECTED, &on_wifi_disconnect) );
    ...
    while (1) {
        EventBits_t eventbits = xEventGroupWaitBits(s_wifi_event_group, CONNECTED_BIT | ESPTOUCH_DONE_BIT, true, false, portMAX_DELAY); 
